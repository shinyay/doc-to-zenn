...
4. **v2 アップデート** ─ route モード (Haiku 4.5 / Sonnet 4.5 / Opus 4.7 のラダー)、CLI ラッパー、12 本の特化 reference を追加した経緯と効果

「動かなくて困ったから直した」ではなく、「**特長を最大限引き出したいから設計した**」というアプローチの記録です。

> 💡 この記事の **前半 (本文 1〜6 章)** は **GitHub Copilot から Opus 4.7 を使うこと** を前提としています。Claude Code (Anthropic 純正 CLI) には触れません。**v2 アップデートセクション以降** では、Claude Code Auto Mode / Stop Hooks / Anthropic Messages API も補足として扱います。
...
スキルには 2 つのモード (執筆当初) + **v2 で追加した 1 つ** があります。

| モード | 用途 |
|---|---|
| **`generate`** | 0 から 5 スロットブリーフを生成する |
| **`audit`** | 既存プロンプトを 4.7 観点で監査し、deprecated パターンを指摘する |
| **`route`** (v2 追加) | Haiku 4.5 / Sonnet 4.5 を first-pass にして、必要なときだけ Opus 4.7 にエスカレーションする「ルーティング・プロンプトパッケージ」を生成する |

`route` モードについての詳細は本記事末尾の「アップデート (v2)」を参照してください。
...
> v2 で 13 → 35 ファイルに拡張しました。`templates/route.md`、12 本の特化 reference、4 つの model profile、offline CLI ラッパー、4 つの fixture が増えています。詳細は本記事末尾の「アップデート (v2)」を参照してください。
...
> v2 で linter を全面書き換えしました。`[ERROR]` / `[WARN]` のカテゴリラベル付き出力、終了コード 0 (clean / warnings のみ) / 1 (hard errors) / 2 (bad invocation)、Anthropic 固有のルール (removed parameters / legacy thinking mode / unsafe CLI flag / GPT leakage / Auto Mode forced-model claim 等) と route 系のルールが入っています。詳細は「アップデート (v2)」セクションを参照。
...
## アップデート (v2): route モード・CLI ラッパー・特化参照 12 本

ここまで紹介してきた `claude-prompt-optimizer` は v1 の時点の構成です。記事公開後、姉妹リポジトリ [`gpt-prompt-optimizer`](https://github.com/shinyay/gpt-prompt-optimizer) を作る過程で、**Claude 側にも持ち込みたくなった構造改善**がいくつか見つかりました。それを v2 として反映しました。
...
|---|---|---|
| ファイル数 | 13 | **35** |
| references | 5 | **22** (+ Anthropic-specific surface 12 本) |
| model profiles | なし | **4** (`claude-opus-4-7` / `claude-sonnet-4-5` / `claude-haiku-4-5` / `claude-code-auto`) |
| templates | 2 (`generate` / `audit`) | **3** (+ `route`) |
| scripts | 1 (`lint-prompt.sh`) | **6** (+ offline CLI + 4 fixtures + linter 全面書き換え) |
| audit findings 形式 | `<text> → <why> → <fix>` | **`severity` / `category` / `offending_text` / `why_it_matters` / `fix`** (構造化) |
| 監査タクソノミー | フリーフォーマット | **約 25 カテゴリの固定タクソノミー** |
| linter 出力 | フリーテキスト | **`[ERROR]` / `[WARN]` カテゴリラベル + 終了コード 0/1/2** |

### 追加 1: `route` モード ─ 安価モデル first → Opus 4.7 escalation

v1 では「常に Opus 4.7 に投げる」前提でした。v2 では **コスト最適化** をプロンプト設計の対象に組み込みます。

`route` モードは次の 4 ブロック契約を出力します。

1. **Routing Strategy** — execution_model / quality_model / budget_priority / cheap_path_scope / escalation_triggers / non_escalation_cases / budget_policy / confidence_gate / handoff_packet_schema
2. **Frontline Prompt** — 安価モデル / Auto Mode 向けの 5 スロットブリーフ
3. **Opus 4.7 Escalation Prompt** — handoff packet を読んで、未解決の hard / risky / ambiguous / high-value な部分だけを解決する 5 スロットブリーフ
4. **API and Claude Code Notes** — Claude Code Auto Mode の caveat、手動オーバーライド条件、Messages API ladder、tool_use schema の使い方、Stop Hooks、prompt caching、privacy/security、cost note

**Claude 系の routing ladder**:

| Tier | スラッグ | 役割 | 用途 |
|---|---|---|---|
| **Haiku 4.5** | `claude-haiku-4-5` | cheap / speed | 分類・要約・simple CRUD・構造化 handoff packet 生成 |
| **Sonnet 4.5** | `claude-sonnet-4-5` | バランス | 中規模実装・bounded code review・first-pass synthesis |
| **Claude Code Auto Mode** | `claude-code-auto` | host 側の **permission モード** | Stop Hooks 連携で bounded autonomous coding |
| **Opus 4.7** | `claude-opus-4-7` | quality / escalation | 複雑推論・高価値コーディング・アーキテクチャ・セキュリティレビュー |

**重要な caveat**: Claude Code の **Auto Mode (Shift+Tab)** は `--dangerously-skip-permissions` を置き換える「**permission の blanket-approval**」であって、**model selector ではありません**。プロンプトに「Auto Mode が Opus 4.7 を選ぶ」と書いても、それは保証されません。これは linter で hard error として弾く設計にしました (`auto-mode-assumption` カテゴリ)。

**Escalation triggers は observable に**: 「if hard」では足りません。`route` モードは次のような **objective triggers** を要求します。

- Validation failure (tests / build / lint / schema / render / smoke が retry 後も fail)
- Evidence gap (必要なソース / ファイル / ログ / API doc が retrieval 予算内で見つからない)
- Security or privacy risk (auth / 権限 / 秘密情報 / 破壊的アクション / PII / sandbox / コンプライアンス)
...
- User override (ユーザーが明示的に Opus 4.7 / 最高品質を要求)

**Handoff packet**: コスト制御の本体です。安価パスがやった作業を全部捨てて Opus 4.7 にゼロから依頼すると、コスト最適化の意味がありません。
...
escalation_reason: "..."
recommended_opus47_focus: [...]
```

Opus 4.7 はこの handoff を読んで **未解決部分だけに集中** します。Anthropic の **`tool_use` schema** で構造化出力させるのが実務上のパターンで、より厳密に shape を保証したいときは **`strict: true`** を付けた strict tool use を使います (詳細は [Anthropic 公式 tool use ドキュメント](https://docs.anthropic.com/en/docs/build-with-claude/tool-use))。

### 追加 2: 12 本の特化 reference (Anthropic 固有 surface)

v1 では `references/` は 6 本 (`presets/opus-4-7.md` + `presets/opus-4-6.md` + `agent-system-prompt.md` + `behavioral-changes.md` + `breaking-changes.md` + `validation-patterns.md`) でした。v2 では Anthropic 固有のサーフェスごとに **12 本の専門 reference** を追加しました。

| 追加 reference | 目的 |
|---|---|
| `extended-thinking-and-effort.md` | Adaptive Thinking、effort levels (low/medium/high/xhigh/max)、`display: "summarized"`、`task_budget` ベータヘッダ |
| `tool-use-and-schemas.md` | `tool_use` の `input_schema`、JSON 強制出力 (Anthropic の "Structured Outputs" 相当)、tool_choice、side-effecting tools |
| `citations.md` | Citations beta、document content blocks、char_location / page_location / content_block_location |
| `prompt-caching-and-context.md` | `cache_control: { type: "ephemeral" }`、5 分 / 1 時間 TTL、`/compact`、コンテキスト管理 |
| `subagents-and-task-tool.md` | Task tool、independent fan-out、4.7 の judicious-subagent ポリシー |
| `claude-code-cli-guidance.md` | Auto Mode、Stop Hooks (`.claude/settings.json`)、`/focus`/`/recap`/`/fewer-permission-prompts`/`/compact`、Plan Mode、MCP |
| `computer-use-and-vision.md` | 画像上限 4.7 で 2576 px / 3.75 MP、Computer Use 座標は **1:1 with actual pixels** (4.6 はスケール) |
| `messages-api-and-managed-agents.md` | Messages API vs Claude Managed Agents vs Anthropic Agent SDK の決定マトリクス |
| `model-routing-and-cost.md` | Routing ladder、budget policy、escalation triggers、handoff packet schema、linter カテゴリ |
| `claude-code-auto-mode-guidance.md` | Auto Mode = permission、model selector ではない、manual Opus 4.7 override の条件 |
| `coding-agent-guidance.md` | Claude Code 向けコーディング・エージェントのプロンプトパック (reuse / sub-agent fan-out / Stop Hooks / validation / security) |
| `frontend-guidance.md` | フロントエンド向け (Playwright MCP screenshots、render checks、design system、responsive) |

`references/validation-patterns.md` も Stop Hooks JSON / forced `tool_use` validation / Citations / Computer Use screenshot validation / sub-agent fan-out validation / fallback validation を含めて拡充しました。

### 追加 3: offline CLI ラッパー (`scripts/optimize-prompt.sh`)

Copilot や Claude Code が無い環境でも、preset + template + payload + (route 時は) routing profile を deterministic に組み立てて貼り付け可能にする **dependency-free Bash ラッパー**を入れました。

```bash
# generate
echo "Add JWT auth to my Express app" | scripts/optimize-prompt.sh generate opus-4-7 -

# audit
scripts/optimize-prompt.sh audit opus-4-7 - < existing-prompt.md

# route (Sonnet 4.5 first / Opus 4.7 escalation)
echo "Plan an auth migration" | scripts/optimize-prompt.sh route \
  --execution-model claude-sonnet-4-5 --quality-model claude-opus-4-7 \
  --budget balanced --api-notes -

# route (Haiku 4.5 cheap-only)
echo "Summarize logs" | scripts/optimize-prompt.sh route \
  --execution-model claude-haiku-4-5 --no-escalation -
```

ラッパーは **オフラインで再現可能** な構成 = **テストしやすい / CI に組み込みやすい** 設計です。LLM 呼び出しは行いません。

### 追加 4: 監査タクソノミーの形式化と強化された linter

v1 の audit はフリーフォーマット (`<text> → <why> → <fix>`) でした。v2 では機械可読な構造に固定しました。
...
- **error**: `removed-parameter` (`temperature` / `top_p` / `top_k` / `budget_tokens`)、`legacy-thinking-mode` (`thinking: { type: "enabled" }`)、`unsafe-cli-flag` (`--dangerously-skip-permissions`)、`gpt-leakage` (OpenAI Responses API 構文の混入)、`auto-mode-assumption` (Auto Mode が GPT-5.5 / Opus 4.7 を強制選択する想定)
- **warning**: `pair-programming-overfit`、`subagent-overuse`、`effort-mismatch`、`negative-style-instruction`、`tokenizer-budget-stale`、`prompt-only-json-schema`、`hidden-chain-of-thought`、`missing-goal` / `missing-acceptance-criteria` / `missing-validation` / `missing-stop-hook`、`unbounded-retrieval`、`citation-risk`、`cache-key-instability`、`current-date-noise`、`auto-mode-misuse` (タクソノミー上は定義済みだが linter での自動検出は将来追加予定)、加えて **`route` 系 8 カテゴリ** (`missing-routing-policy` / `missing-escalation-criteria` / `missing-handoff-packet` / `premium-overuse-risk` / `cheap-model-overreach` / `missing-budget-policy` / `missing-confidence-gate` / `unbounded-escalation-loop`)

linter は次の終了コードガードレールを持ちます。

| 出力 | 意味 | 終了コード |
|---|---|---|
| `[OK] Clean` | warnings ゼロ | **0** |
| `[WARN] Lnnn category: ...` | パスはするが弱い | **0** (warnings のみ) |
| `[ERROR] Lnnn category: ...` | hard incompatibility | **1** |
| `Usage: ...` | bad invocation | **2** |

加えて、**4 つの fixture** を同梱しています。

| fixture | 期待 exit code |
|---|---|
| `clean-opus-4-7.prompt.md` | 0 |
| `bad-legacy.prompt.md` | 1 |
| `clean-route.prompt.md` | 0 |
| `bad-route.prompt.md` | 1 |

これにより、新ルール追加時に偽陽性 / 偽陰性を **CI で継続的に検知** できる構造になりました。

### 何がどう良くなったか

| 効果 | 具体 |
|---|---|
| **読みやすい audit finding** | `[ERROR] L8 unsafe-cli-flag: ...` のように機械的に grep できる |
| **コスト最適化のフロー** | 「単純なら Sonnet 4.5、難しければ Opus 4.7」をプロンプトレベルで固定化 |
| **CI に組み込めるラッパー** | LLM を呼ばない deterministic な組み立てなので、`make test` で fixtures を回し続けられる |
| **Anthropic surface の網羅性** | Adaptive Thinking / `cache_control` / Citations / Computer Use 等、4.7 で意識すべきサーフェスが必要に応じて参照される |

### v2 で「直したくなった」課題 ─ rubber-duck で 19+4 件

v2 への改修は、姉妹リポジトリ `gpt-prompt-optimizer` を作る過程で **rubber-duck 専用の sub-agent に critique** させて見つかった課題が引き金でした。19 件指摘 → 修正 → 再 critique で 4 件の follow-up → 全部解決、という流れで **0/1/0/1** の fixture 終了コードを保ったまま整理しました。

このプロセス自体が `claude-prompt-optimizer` の v2 にも持ち込み価値があると判断し、同じ構造改善を Anthropic 流に翻訳して入れたのが v2 の中身です。

> 💡 **ポイント**: v2 の各追加は **「Claude 側で必要な surface だから入れた」** と **「GPT 側で発見した良い構造を Anthropic 流に翻訳して入れた」** の両方があります。「LLM ごとの特長に合わせる」設計は cross-model な学びを跨ぐと加速する、というのが v2 を通じての発見です。
...
4. **route モードでコストと品質をデザインする** (v2) ─ 単一モデル前提を捨て、Haiku 4.5 / Sonnet 4.5 / Opus 4.7 の役割分担をプロンプトに埋め込む
...
- [gpt-prompt-optimizer](https://github.com/shinyay/gpt-prompt-optimizer) (姉妹リポジトリ; v2 改修の起点)
- [GPT-5.5 にも『指示の型』を ─ 3 モード Agent Skill とコスト最適化ルーティング](./gpt-5-5-prompt-optimization-copilot-skill) (GPT-5.5 版の同型記事)
