...
Six main input segments + one optional auxiliary segment + the output, in roughly the order they appear in the request:
...
### 7. (Optional) App-side reasoning scaffold / scratchpad / chain-of-thought hints

When applicable, the request includes **app-supplied** reasoning scaffolding: scratch space the app threads in for the model to "think" against, records of continuing prior reasoning, structured templates for internal reasoning. **Not all models support this and not all apps use it.**

- Sent only when the app opts in
- Size depends on technique — negligible to substantial
- This is **distinct from model-generated hidden reasoning tokens (which live on the output side)** — segment 7 refers to **app-supplied scaffolding that rides on the input side** (see the Output section below for the model-generated kind)

### And: the output

The model's reply. On some models, this includes **hidden reasoning tokens that are billed but not displayed**. Those are **generated by the model** and are separate from the app-supplied scaffolding in segment 7.
...
When a tool returns output, that output has to get back to the model somehow. Most providers **append the tool result as a special message in the conversation** → it becomes **part of input on the next step**. **A tool that returns a 5,000-token file keeps adding 5,000 input tokens on every subsequent step until that piece of history is dropped or summarized.** If the volatile portion stays at the very end so that prompt caching can hit, the **price** of those 5,000 tokens may be discounted on later turns — but **the token count itself, and the context-window pressure, remain.**

Most common surprise in agent cost analysis: a single early-session "read this big file" tool call **continues to ride along in context and counts for the rest of the session**.
